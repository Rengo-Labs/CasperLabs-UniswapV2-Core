...
fn remove_liquidity_with_permit() {
    let (env, uniswap, owner, router_contract, flash_swapper ,pair_contract, token1, token2, token3, _, factory) =
        deploy_uniswap_router();
...
fn swap_exact_tokens_for_tokens() {
    let (env, uniswap, owner, router_package_hash, _ , _, token1, token2, token3, _, _) =
        deploy_uniswap_router();

    let amount_in: U256 = 400.into();
