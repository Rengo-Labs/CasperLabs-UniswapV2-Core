...
fn test_uniswap_deploy() {
    ...
    let (env, token, owner, router_contract, _, _, _, _, _, factory) = deploy_uniswap_router();
...
#[test]
fn add_liquidity() {
    let (env, uniswap, owner, router_contract, flash_swapper _, token1, token2, token3, _, _) =
        deploy_uniswap_router();


    let router_package_hash: ContractPackageHash = router_contract.query_named_key(String::from("package_hash"));
    let router_package_hash: Key = router_package_hash.into();
    let pair: TestContract = deploy_pair_contract(&env, owner, factory, flash_swapper);
