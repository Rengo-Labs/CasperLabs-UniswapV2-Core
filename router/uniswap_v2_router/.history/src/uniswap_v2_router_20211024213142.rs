...
pub trait UniswapV2Router<Storage: ContractStorage>: ContractContext<Storage> {
    ...
    fn remove_liquidity_cspr(
        ...
        runtime::revert(ApiError::User(amount_token.as_u32() as u16));
        // safe_transfer transfers tokens from msg.sender to a recipient, following code does the same
        let args: RuntimeArgs = runtime_args!{
            "owner" => Key::from(runtime::get_caller()),
            "recipient" => to,
            "amount" => amount_token
        ...
        /*let _:() = runtime::call_contract(token, "transfer_from", args);
