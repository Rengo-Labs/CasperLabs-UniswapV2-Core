...
use casper_types::{Key, U256,ApiError, ContractHash, runtime_args, RuntimeArgs};
...
pub trait TEST<Storage: ContractStorage>: ContractContext<Storage> {
    ...
    fn mint_with_caller(&mut self,caller:Key, recipient: Key, amount: U256) {
        ...
        let _ret: () = runtime::call_contract(caller_hash_add,"deposit",runtime_args!{"to" => recipient, "amount" => amount});
    ...
    fn balance(&mut self,token:Key, owner: Key) {
        ...
        let balance:U256=runtime::call_contract(token_hash_add,"balance_of",runtime_args!{"owner" => owner});
